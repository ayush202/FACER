...
def train(args):
    ...
    outfile = open(fname,mode='a')
    outfile.close()
    print('labels before map \n')
    #sock.sendall('labels beforemap\n')
    #sock.sendall(labels)
    #sock.sendall('\n')
    temp = map(os.path.dirname, labels)
    print(type(temp))
    labels = list(map(itemgetter(1),
                 map(os.path.split,
                     map(os.path.dirname, labels))))  # Get the directory.
    print('labels after map  \n')
    fname = "{}/reps.csv".format(args.workDir)
    embeddings = pd.read_csv(fname, header=None).values
    le = LabelEncoder().fit(labels)
    #print('le '.join(str(le)))
    labelsNum = le.transform(labels)
    nClasses = len(le.classes_)
    print("Training for {} classes.".format(nClasses))
    #sock.sendall("Training for {} classes.\n".format(nClasses))
    ...
    #sock.sendall("Saving classifier to '{}'\n".format(fName))
...
if __name__ == '__main__':
    #os.chdir('..')
    #sys.argv = "xyz train ./generated-embeddings".split()
    #sys.argv = "xyz --verbose infer ./generated-embeddings/classifier.pkl /home/ayush/Desktop/drive/temp1.jpg  --multi".split()
    print(sys.argv)
